...
bundle-devkit 是一个 pnpm workspace monorepo，包含 11 个 `@devkit/` scoped 包（devkit-cli、devkit-service、devkit-bundler-*、devkit-plugin-*），使用 Turborepo 编排构建任务，发布到内部私有 npm registry（`https://registry.example.com/`）。
