...
- [x] 1.1 在 `packages/bundlekit-shared-utils/lib/shared/` 新增 `confirm.ts`，导出 `confirm({ message, default? }): Promise<boolean>`，基于 `enquirer` 实现
- [x] 1.2 `confirm` 内部判定 `process.stdout.isTTY` 与 `process.env.DEVKIT_NO_PROMPT`，非交互场景返回 `default ?? false`
- [x] 1.3 在 `packages/bundlekit-shared-utils/lib/shared/index.ts` 导出 `confirm`
- [x] 1.4 单测：在 `__tests__` 下覆盖 TTY / 非 TTY / DEVKIT_NO_PROMPT 三个分支

## 2. shared-utils: 类型 & 常量

- [x] 2.1 新增 `lib/types/cli-init/` 中 `BUNDLER_PACKAGE_MAP` 常量类型，列出 `webpack|vite|rspack|rollup|rolldown → @bundlekit/bundler-*`
- [x] 2.2 导出 `BUNDLER_PACKAGE_MAP` 与配套 `IBundlerName` 类型，供 cli 与 service 共享

## 3. service: 移除 bundler-* 依赖

- [x] 3.1 修改 `packages/bundlekit-service/package.json`：从 `dependencies` 移除 5 个 `@bundlekit/bundler-*`
...
- [x] 4.5 用户拒绝 / 安装失败时，输出包含 `bundlekit-cli add bundler-<name>` 引导文案，`process.exit(1)`
...
- [x] 5.2 cli 渲染模板时把所选 bundler 注入 `package.json.ejs`（让 `devDependencies` 中包含一行 `"@bundlekit/bundler-<%= bundler %>": "<%= bundlerVersion %>"`），或者直接在生成后用 `fs` 改 package.json — 二选一，design D4 倾向后者
- [x] 5.3 版本号由 `cli/package.json` 的 `dependencies` 中查到（同时把 cli 自身 deps 也补上 5 个 `@bundlekit/bundler-*` 仅用于版本范围声明）
- [x] 5.4 同步更新 `packages/bundlekit-plugin-react/templates/template-react-{ts,js}/package.json.ejs` 与 `template-vue3-{ts,js}`：去除（如果有）写死的 bundler 行，改由 cli 注入
- [x] 5.5 验证 `dc create demo -b vite` 后 demo 项目的 package.json 含 `@bundlekit/bundler-vite`，且无其他 bundler
...
- [ ] 6.4 验证 `dc add vite`、`dc add bundler-rspack`、`dc add @bundlekit/bundler-rollup` 三种写法等价
- [x] 6.5 验证 add bundler 后**不**自动调用 generator（与 plugin 区分），只装包

## 7. 模板与发版

- [x] 7.1 在 `cli/package.json` 的 `dependencies` 中显式列出 5 个 `@bundlekit/bundler-*: workspace:*`（仅用于版本范围 / 发版时被替换为 caret）
...
- [x] 8.1 在 `packages/bundlekit-docs/docs/guide/bundlers.md` 顶部加一节 "Bundler 安装方式"，说明三种入口（create/add/runtime prompt）
- [x] 8.2 在 `packages/bundlekit-docs/docs/guide/cli.md` 的 `add` 章节追加 `bundler-*` 短名表
...
- [ ] 9.2 在 monorepo 外 `pnpm pack` 后 npm install 模拟用户场景，验证 `@bundlekit/service` install size 显著下降
- [x] 9.3 模拟 CI：`DEVKIT_AUTO_INSTALL=1` 下能跑通；不设变量时报错引导
- [ ] 9.4 模拟用户切 bundler：`pnpm install` 后改 `.bundlekitrc.ts` 的 `bundler` 字段，运行 `serve` 时被 prompt 安装并能继续
