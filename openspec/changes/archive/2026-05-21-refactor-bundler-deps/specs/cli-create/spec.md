...
- **WHEN** user runs `bundlekit-cli create my-app --template react-ts --bundler vite`
- **THEN** files SHALL be generated in `./my-app`
- **AND** `./my-app/package.json` SHALL list `@bundlekit/bundler-vite` under `devDependencies`
- **AND** dependencies SHALL be installed in a single run (covering `@bundlekit/service`, `@bundlekit/bundler-vite`, framework plugin, and template runtime deps)
...
- **WHEN** user runs `bundlekit-cli create my-app --template react-ts` without `--bundler`
