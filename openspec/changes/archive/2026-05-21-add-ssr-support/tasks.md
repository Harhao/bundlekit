...
- [x] 2.5 实现 dev SSR HTTP server orchestrator：`startSSRDev(buildConfig, adapter, ctx)`：用 `connect` 串 middleware，绑定 `host:port`
- [x] 2.6 实现 SSR HTML render 工具：读取 template → 调用 adapter HTML 转换（如 vite transformIndexHtml）→ require server bundle → 调用 render(url) → 替换 placeholder
- [x] 2.7 SSR 错误处理：dev 返回 500 + stack overlay，build 抛出后 exit(1)
...
- [x] 3.3 验证 client HMR + server HMR 都能工作
- [x] 3.4 测试：react-ts demo build & dev SSR

## 4. webpack adapter SSR

- [x] 4.1 引入 `webpack-node-externals` 依赖
- [x] 4.2 在 `transformConfig` 中根据 `ctx.env === 'server'` 切换 `target / output.libraryTarget / externals`
- [x] 4.3 引入 `webpack-dev-middleware` 与 `webpack-hot-middleware`
- [x] 4.4 实现 `createSSRMiddleware`：返回 `[devMiddleware, hotMiddleware, ssrHandler]`，server 编译走单独 watcher
- [x] 4.5 实现 server bundle require cache invalidation：每次请求或 watcher 重建后 `delete require.cache[serverBundlePath]`
...
- [x] 5.2 实现 `createSSRMiddleware`：用 `@rspack/dev-server` 的 middleware mode + ssrHandler
...
- [x] 6.2 实现 `createSSRMiddleware`：rollup `watch()` + ssrHandler，无 HMR 注入
...
- [x] 7.2 实现 `createSSRMiddleware`：rolldown watch + ssrHandler，无 HMR
...
- [x] 10.2 vite/webpack/rspack dev SSR：curl `localhost:3000` 拿到注水 HTML，浏览器 HMR 工作
- [x] 10.3 rollup/rolldown dev SSR：curl 拿到 HTML，编辑文件后再次 curl 看到更新
