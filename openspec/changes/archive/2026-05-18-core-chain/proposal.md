...
The core build chain (ConfigLoader → Service → Bundler adapters) is the backbone of bundle-devkit. Currently ConfigLoader is fully empty, Service has multiple TODOs preventing plugin/bundler resolution, serve command option parsing is commented out, and Webpack/Vite config transformation doesn't properly consume `IBuildConfig`. Without a working core chain, no project can actually build or serve.

## What Changes

- **ConfigLoader**: Implement loading `.devkitrc.ts`/`.devkitrc.js` via `jiti`, merge with default config
...
### New Capabilities
- `config-loading`: Load and merge project config from `.devkitrc.ts`/`.devkitrc.js` with defaults
...
- Affected packages: `@devkit/service`, `@devkit/bundler-webpack`, `@devkit/bundler-vite`, `@devkit/shared-utils`
