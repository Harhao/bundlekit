...
The system SHALL resolve bundler adapters by looking up `@bundlekit/bundler-{name}` packages, first checking local `node_modules`, then fetching from a remote registry URL with local cache fallback.
