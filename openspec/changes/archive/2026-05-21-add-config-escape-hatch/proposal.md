...
当某个 `@devkit/bundler-*` 适配器没有实现用户需要的能力（缺某个 plugin、某种 loader、某个 webpack 性能选项等），用户当前唯一的扩展点是顶层的 `changeConfigure(config, mode)` 全局钩子（`Service.ts:180-187`）。但它有两个体感问题：
...
- 在每个 `@devkit/bundler-*` 适配器的 `transformConfig` 完成后、`run` 之前调用对应的 `tools[bundler]?.(config, ctx)`。
...
**代码变更**
- `packages/devkit-shared-utils/lib/types/cli-service/config.ts`：新增 `tools` 字段类型 + `ToolsCtx`
- `packages/devkit-shared-utils/lib/types/cli-service/adapter.ts`：扩展 `IBuildToolAdapter` 接口（可选）暴露当前 bundler 名，让 service 选择 hook
- `packages/devkit-service/lib/Service.ts`：`startBuilder` 在 `transformConfig` 与 `configureConfig` 之间插入 `applyTools(buildConfig, bundlerName, config)`
- 每个 `packages/devkit-bundler-*/src/index.ts`：在 transform 后回调 service 注入的 hook（推荐通过 service 在 startBuilder 内统一调用，bundler 适配器无需感知）
...
**风险**
- TypeScript 类型映射（不同 bundler config 类型不同）会让 `IBuildConfig` 的类型签名变复杂；为保证用户写 `.devkitrc.ts` 时 IDE 推断正确，需在 shared-utils 中重新导出各 bundler 的 config 类型，间接引入对各 bundler 包的类型依赖（不是运行时依赖）。
