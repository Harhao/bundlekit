...
- [x] 1.1 在 `packages/bundlekit-shared-utils/lib/types/cli-service/config.ts` 新增 `ToolsCtx` 接口（`mode / command / env / bundler`）
...
- [x] 1.5 单测：`.bundlekitrc.ts` 类型断言 fixture，覆盖 5 个 bundler 钩子的类型推断

## 2. service: 注入 hook

- [x] 2.1 新增 `packages/bundlekit-service/lib/utils/applyTools.ts`：导出 `applyTools(buildConfig, bundlerName, rawConfig, ctx): Promise<unknown>`，处理同步/异步、void/return 两种返回形态
...
- [x] 4.1 在 `packages/bundlekit-docs/docs/guide/config.md` 新增"逃生舱（tools）"章节，给出 5 个 bundler 的最小用例
...
- [x] 5.2 在 example 项目中加一个 `.bundlekitrc.ts` 用例，包含 `tools.webpack`，运行 build 验证 plugin 被注入
