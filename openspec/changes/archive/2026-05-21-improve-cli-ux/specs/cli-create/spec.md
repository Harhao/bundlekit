...
- **WHEN** user runs `devkit-cli create my-app` without `--template` in a TTY
- **THEN** the system SHALL render an ink `<SelectInput>` with the four available templates

#### Scenario: No template specified, non-TTY
- **WHEN** user runs `devkit-cli create my-app` without `--template` in a non-TTY environment
...
- **WHEN** user runs `devkit-cli create my-app --template react-ts --bundler vite` in a TTY
...
- **WHEN** user runs `devkit-cli create my-app --template react-ts --bundler vite` in CI (non-TTY)
...
- **WHEN** user runs `devkit-cli create my-app --template react-ts` without `--bundler` in a TTY
