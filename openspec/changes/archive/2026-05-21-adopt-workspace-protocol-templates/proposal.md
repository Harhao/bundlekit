...
当前 cli 创建项目时卡在依赖安装步骤，根因是模板生成的 `package.json` 把 `@bundlekit/service` / `@bundlekit/plugin-react` / `@bundlekit/plugin-vue` 等内部包硬编码为固定语义化版本（`^0.0.1` / `^1.0.0`），但这些包**并未发布到 npm registry**，导致任何 `pnpm/npm/yarn install` 都会失败或长时间 hang。
...
- **模板：4 个 `package.json.ejs` 把 `@bundlekit/*` 内部包改写为 `workspace:^`**
...
- **cli：`renderTemplates` 后新增"依赖版本规范化"步骤**
  - 检测当前 cwd 是否在 monorepo 内（`pnpm-workspace.yaml` + `packages/bundlekit-service` 双重判定）
  - 在 monorepo 内 → 把生成项目 package.json 的 `workspace:^` 替换为 `link:/abs/path/to/packages/bundlekit-{name}`
...
- **plugin 包：`@bundlekit/plugin-react` / `@bundlekit/plugin-vue` 版本归一**
...
- **代码**：
  - `packages/bundlekit-cli/lib/utils/projectMutation.ts`：新增 `findMonorepoRoot` / `resolveDepMode` / 替换 helper
  - `packages/bundlekit-cli/lib/commands/create/actions.ts`：`renderTemplates` 后调依赖规范化
  - `packages/bundlekit-cli/lib/commands/create/creator.ts`：legacy 路径同款
  - 4 个 `package.json.ejs` 模板
  - `packages/bundlekit-plugin-react/package.json` 版本归一
  - `packages/bundlekit-plugin-vue/package.json` 版本归一
- **文档**：
  - `packages/bundlekit-docs/docs/contributing/`（新目录，6 个文件）
  - `packages/bundlekit-docs/.dumirc.ts`（导航 + sidebar）
  - `packages/bundlekit-docs/docs/guide.md` / `cli.md` 内容更新
- **CI**：
