...
- **WHEN** the CLI runs from inside the devkit monorepo
- **THEN** every `@devkit/*` entry in the generated `package.json.devDependencies` SHALL start with `link:` followed by an absolute path
- **AND** no `workspace:^` literal SHALL appear anywhere

#### Scenario: Internal deps normalized outside monorepo
- **WHEN** the CLI runs from outside any devkit monorepo (or with `DEVKIT_DEP_MODE=npm`)
- **THEN** every `@devkit/*` entry in the generated `package.json.devDependencies` SHALL be `^${cliVersion}` where `cliVersion` is the CLI's own package version
- **AND** no `workspace:^` literal SHALL appear anywhere
