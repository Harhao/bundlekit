...
1. **`@devkit/plugin-react` 的 generator 在 install 之后弹了一个 enquirer prompt**（"是否同时安装 @devkit/request HTTP 客户端？"），但前一步是 `Spinner` 转动 + `setBinaryMirrors` 的 `ERR_INVALID_PROTOCOL` stderr 噪音，把 prompt 文案淹没，用户看不到提示就以为 cli hang 了
2. **prompt 在 ink TTY 路径下也会弹**，导致 ink 渲染 frame 与 enquirer 的 raw 模式 stdin 抢占冲突
3. **plugin-react generator 写的依赖版本 `^1.0.0` 是死硬编码**，违反 lockstep 约定，且 `@devkit/request` 当前版本是 `0.0.1`，写 `^1.0.0` 装不到任何东西
4. **`PackageManager.install` 在 monorepo 子目录运行时不传 `--ignore-workspace`**，依赖 `link:` 协议侥幸绕开 workspace 解析，但属于脆弱实现，外部用户在自己的 monorepo 内创建 devkit 项目会立刻撞坑
...
- **`@devkit/plugin-react` generator**：
  - 在非 TTY、`DEVKIT_NO_PROMPT=1`、`CI=true` 任一条件下默认跳过 prompt（不安装 @devkit/request）
  - prompt 渲染前显式 `console.log("\n")` 加一个换行，避免被 spinner 残留遮蔽
  - 把硬编码 `^1.0.0` 改成 `workspace:^`（让 cli 的 `normalizeDeps` 兜底）
- **`@devkit/plugin-vue` generator**：相同改造（保持一致性）
- **`@devkit/cli` ink 路径**：在 `runGenerator` 之前注入 `process.env.DEVKIT_NO_PROMPT = "1"`，避免 generator 在 ink 渲染中抢 stdin
- **`@devkit/shared-utils` PackageManager**：
...
- **`@devkit/cli`**：generator 调用前后加显式视觉分隔符，让 prompt 文案更醒目
...
- **代码**：
  - `packages/devkit-plugin-react/generator/index.ts`：prompt 条件 + 版本注入
  - `packages/devkit-plugin-vue/generator/index.ts`：同款（如存在）
  - `packages/devkit-cli/lib/ui/CreateApp.tsx`：runGenerator 前注入 DEVKIT_NO_PROMPT
  - `packages/devkit-cli/lib/commands/create/creator.ts`：legacy 路径同款
  - `packages/devkit-shared-utils/lib/shared/pkgManager.ts`：`--ignore-workspace` 检测 + 静默 binary-mirror error
- **测试**：
