...
- [ ] 2.3 加入 ink UI 截图（asciinema 转 SVG，存放 `docs/assets/cli-create.svg`）
...
- [ ] 4.3 在 Vite / Webpack / Rspack 节末尾追加各自 SSR 行为的简介
...
- [ ] 5.4 在"构建依赖顺序（Turbo）"中提示 service:build 的 dependsOn 变化
...
- [x] 6.3 HMR 支持矩阵表格
- [ ] 6.4 react-ts 完整迁移示例（从 CSR 改 SSR 的 diff）
- [ ] 6.5 vue3-ts 迁移示例
...
- [ ] 7.4 验证 dumi `pnpm --filter @devkit/docs build` 能跑通，无 broken link
