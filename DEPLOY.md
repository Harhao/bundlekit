...
bundle-devkit 用 [Changesets](https://github.com/changesets/changesets) + [GitHub Actions](https://docs.github.com/en/actions) 实现自动化 npm publish。本文档记录从零到首发到迭代的完整步骤。
...
```bash
npm view @devkit/cli 2>&1 | head -3
# 期望：404 Not Found（说明 scope 没被占）
```

如果 `@devkit` scope 已被别人占，需要把所有 `@devkit/*` 包改名（例如改成 `@bundle-devkit/*` 或 `@<你的用户名>/devkit-*`）。涉及修改：

- 各 `packages/*/package.json` 的 `name`
- `packages/devkit-shared-utils/lib/types/cli-init/index.ts` 的 `BUNDLER_PACKAGE_MAP`
- `packages/devkit-plugin-*/templates/*/package.json.ejs` 中的 dep 名
- 全部文档（docs / README）中的 import 示例

### 步骤 2：在 npm 创建 organization

访问 https://www.npmjs.com/org/create，新建 `@devkit` org（如果是个人 scope 直接用账号 scope 即可）。
...
3. 配置：
   ...
   | 字段 | 值 |
   |---|---|
   | Token name | `bundle-devkit-ci` |
   | Expiration | 365 天（到期前要轮换） |
   | Packages and scopes | 勾选 `@devkit` scope 下全部包 |
...
1. 打开仓库 https://github.com/Harhao/bundle-devkit → **Settings** 标签
...
bundle-devkit 当前所有包是 `0.0.1`，首发推荐 bump 到 `0.1.0`（pre-1.0 不稳定语义，给 BREAKING 留余地）。
...
```
🦋  Releases:
🦋    @devkit/cli@0.1.0 (minor)
🦋    @devkit/service@0.1.0 (minor)
🦋    ...
```
...
打开 https://github.com/Harhao/bundle-devkit/actions 看 `publish-npm` workflow：
...
- ✅ 每个 `@devkit/*` 包的 `package.json.version` 从 `0.0.1` 改为 `0.1.0`
...
npm view @devkit/service
npm view @devkit/bundler-vite
npm view @devkit/plugin-react
# 全部都应该是 0.1.0
```

也可以打开 https://www.npmjs.com/package/@devkit/cli 看 release。
...
# 全新目录
cd /tmp/devkit-test
npm install -g @devkit/cli@0.1.0
...
bundle-devkit 用 **lockstep 发版**（所有 `@devkit/*` 同步 bump），原因：

- cli 创建项目时模板用 `workspace:^` 协议
- pnpm publish 自动把 `workspace:^` 转换为依赖包的实际版本
- 假设所有 `@devkit/*` 版本同步，避免 service@0.1.0 拿到 plugin-react@0.0.5 这种错配

```
✓ 正确  → 所有 @devkit/* 同时 minor bump（0.1.0 → 0.2.0 全套）
✗ 错误  → 单独把 @devkit/plugin-react bump 到 0.5.0
```
...
2. Token 是否对 `@devkit` scope 有 read+write 权限
...
npm deprecate @devkit/cli@0.1.0 "Bad release, use 0.1.1"
...
+ "@devkit/service": "^0.1.0"
```

转换基于 monorepo 里 `@devkit/service` 当前版本号。
...
1. 看哪些包成功发了（`npm view @devkit/<pkg>` 验）
2. 把失败的包手工发：
   ```bash
   cd packages/devkit-<failed>
...
- [ ] 所有 `@devkit/*` 包版本号统一
- [ ] 所有 `@devkit/*` 包 `publishConfig.registry` 正确
- [ ] 所有 `@devkit/*` 包 `files` 字段不含源码
...
- [贡献指南](./packages/devkit-docs/docs/contributing/index.md) — 完整 contributor 流程
- [发版流程文档站](./packages/devkit-docs/docs/contributing/release.md) — 同内容的文档站版本
