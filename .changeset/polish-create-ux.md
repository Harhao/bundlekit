...
- **变更（@bundlekit/plugin-react、@bundlekit/plugin-vue）**：`template-react-ts` / `template-react-js` / `template-vue3-ts` / `template-vue3-js` 的 `package.json` 模板精简，仅生成 `clean` / `dev` / `build` 三个脚本；删除 `${bundler}:dev` / `${bundler}:prod` 等 bundler 专属别名（与 `dev` / `build` 完全等价的冗余项）。
