...
import type { Logger } from "@devkit/shared-utils";
...
const LIVERELOAD_SNIPPET = `<script data-devkit="livereload">
(function () {
  var sse = new EventSource('/__devkit_sse');
  sse.addEventListener('message', function (e) {
    if (e.data === 'reload') {
      console.log('[devkit] livereload triggered');
      location.reload();
    }
  });
    ...
    public start(): Promise<void> {
        return new Promise((resolve, reject) => {
            this.server = http.createServer(async (req, res) => {
                ...
                if (url.startsWith("/__devkit_sse")) {
                    res.writeHead(200, {
                ...
                try {
                    ...
                    this.logger.error(`[devkit] 读取文件失败: ${err.message}`);
                    res.writeHead(500);
    ...
    private async handleProxy(
        ...
        for (const [prefix, rule] of Object.entries(proxy)) {
            ...
            try {
                ...
                this.logger.error(`[devkit] 代理目标 URL 无效: ${opts.target}`);
                res.writeHead(502);
            ...
            return new Promise<boolean>((resolve) => {
                ...
                proxyReq.on("error", (err) => {
                    this.logger.error(`[devkit] 代理请求失败: ${err.message}`);
    ...
    private openBrowser(url: string): void {
        const cmd =
            ...
            process.platform === "win32"   ? `start "" "${url}"` :
                                             `xdg-open "${url}"`;
        exec(cmd, (err) => {
            if (err) this.logger.warn(`[devkit] 打开浏览器失败: ${err.message}`);
        });
    }
}
