import { PackageManager, Logger, resolveBundlerName, BUNDLER_PACKAGE_MAP } from "@devkit/shared-utils";
...
function isBundlerPackage(pkgName: string): boolean {
    return /^@devkit\/bundler-/.test(pkgName);
}

function resolvePackageName(input: string): string {
    // 1) 优先按 bundler 短名解析（vite / bundler-vite / @devkit/bundler-vite）
    const bundlerPkg = (() => {
