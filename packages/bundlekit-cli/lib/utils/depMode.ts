...
/**
 * 读取 @bundlekit/cli 自身的版本号（用于 ^${cliVersion}）
 *
 * 实现策略：
...
export function readCliVersion(): string {
    try {
        ...
        const pkgJsonPath = req.resolve("@bundlekit/cli/package.json", {
            paths: [process.cwd()],
        });
...
/**
 * 给定 @bundlekit/<full> 完整包名，返回 short pkg 用于 link 路径拼接
 *   "@bundlekit/service"        → "service"
 *   "@bundlekit/plugin-react"   → "plugin-react"
 *   "@bundlekit/bundler-vite"   → "bundler-vite"
 */
export function shortNameFromFullPkg(fullPkg: string): string | null {
    const m = /^@bundlekit\/(.+)$/.exec(fullPkg.trim());
    return m ? m[1] : null;
}

/**
 * 把 targetDir/package.json 中所有 @bundlekit/* 依赖的 "workspace:^" 字面量
...
export function normalizeDeps(targetDir: string, mode: IDepMode): INormalizeDepsResult {
    ...
    for (const block of blocks) {
        ...
        for (const fullPkg of Object.keys(deps)) {
            ...
            if (!short) {
                // 非 @bundlekit/* 的 workspace:^（理论上不该出现），保守删除 workspace: 前缀
 ...
 * devDependencies 中的 @bundlekit/bundler-{name} 字段。
 *
