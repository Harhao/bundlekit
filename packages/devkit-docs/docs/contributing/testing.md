...
- `configLoader.test.ts` — `.devkitrc.ts` 加载与默认值合并
...
│   ├── spawnService.ts     # spawn devkit-service 子进程
...
│   └── <bundler>/          # 各 bundler 自己的 .devkitrc + package.json
...
```bash
# 1. 在 fixtures/<bundler>/ 已有 .devkitrc.<mode>.ts
