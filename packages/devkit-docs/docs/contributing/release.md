...
bundle-devkit 用 [Changesets](https://github.com/changesets/changesets) 管理版本与 changelog，配合 GitHub Actions 自动 npm publish。
...
1. **Which packages would you like to include?**
   按空格选要 bump 的 `@devkit/*` 包。一般来说改了哪个包就选哪个。
...
4. **Summary**
   ...
   ```
   Added Svelte framework plugin (@devkit/plugin-svelte) with templates for
...
- `access: "public"` — `@devkit/*` scope 包默认私有，必须显式开公开
...
4. 配置：
   ...
   - Packages: 选 `@devkit/*` scope（首次发布时这个 scope 需先在 npm 上 owner 占领）
...
# 在 https://www.npmjs.com/org/create 创建 @devkit org
# 或者改为个人 scope：@<your-username>
```

如果 `@devkit` scope 被别人占了，需要把所有包名改成 `@<your-name>-devkit/...`。改：
...
往后用户可以 `npm install -g @devkit/cli@0.1.0` 拉到。
...
转换基于当前 `@devkit/service` 在 monorepo 里的版本号。
