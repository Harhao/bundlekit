...
# 或者按需构建单个包
pnpm --filter @devkit/cli run cli:build
pnpm --filter @devkit/service run service:build
pnpm --filter @devkit/shared-utils run shared:build
pnpm --filter @devkit/bundler-vite run vite:build
pnpm --filter @devkit/bundler-webpack run webpack:build
pnpm --filter @devkit/bundler-rspack run rspack:build
pnpm --filter @devkit/bundler-rollup run rollup:build
pnpm --filter @devkit/bundler-rolldown run rolldown:build
```
...
> 💡 检测逻辑：cwd 向上找 `pnpm-workspace.yaml` + `packages/devkit-service` 双重判定。设 `DEVKIT_DEP_MODE=npm` 可强制走 npm 模式（用于测试）。

## 启动文档站

```bash
pnpm --filter devkit-cli-docs run dev
...
@devkit/shared-utils
    ↓（被所有包依赖）
@devkit/bundler-{webpack,vite,rspack,rollup,rolldown}
    ↓（运行时由 service 动态加载）
@devkit/service
    ↓（被生成项目依赖）
@devkit/cli      ← 独立，不依赖 service / bundler

@devkit/plugin-{react,vue}   ← 独立，由用户项目自选
@devkit/plugin-mock          ← 独立 dev 工具
@devkit/request              ← 独立 runtime 库
```
...
```bash
pnpm --filter @devkit/<改的包> run <build script>
```
