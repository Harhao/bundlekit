...
features:
  ...
  - title: CLI 脚手架（ink 美化）
    emoji: 🚀
    description: devkit-cli 提供 create / add 两条命令，TTY 终端用 ink 渲染 banner + 步骤式表单，CI 自动回退到行式输出

  - title: 统一配置驱动 + 逃生舱
    emoji: 📋
    description: 通过 .devkitrc.ts 集中管理多环境构建配置；当 bundler 抽象不够用时，tools.{bundler} 钩子直接拿到原生 config 进行扩展
...
cli 会引导你选择模板和 bundler，创建后自动安装 `@devkit/service` + 框架插件 + 你选择的 bundler 适配器。
