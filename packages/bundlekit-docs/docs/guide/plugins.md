...
bundlekit 插件分两类，定位和用法截然不同：

| 类型 | 包名规范 | 配置位置 | 用途 |
|------|---------|---------|------|
| 构建插件 | `@bundlekit/plugin-*` | `.bundlekitrc.ts` `plugins[]` | 在构建阶段修改打包器配置 |
| 运行时库 | `@bundlekit/*`（无 plugin-） | `package.json` `dependencies` | 在前端业务代码中 import 使用 |

---

## 构建插件

构建插件通过 `plugins[]` 声明，bundlekit-service 启动时自动加载并调用 `apply()`。
...
| `api.registerCommand(name, opts, fn)` | 注册自定义 bundlekit-service 命令 |
| `api.addBuildPackage(pkg)` | 安装额外 npm 包 |
| `api.getCwd()` | 获取项目根目录（等同于 `process.cwd()`） |
| `api.pluginName` | 当前插件的 ID（只读） |
| `api.version` | bundlekit-service 的版本号（只读） |
...
```json
{
  ...
  "posts": [
    { "id": 1, "title": "Hello bundlekit", "userId": 1 }
  ]
}
```
...
export default {
  ...
  apply(api: IPluginAPIClass, options: IBuildConfig) {
    ...
    api.registerCommand("my-cmd", {
      description: "我的命令",
      usage: "bundlekit-service my-cmd",
      options: { "--port": "端口号" },
    }, async (args) => {
...
generator 让 `bundlekit-cli add my-plugin` 能自动更新项目配置，并通过 `IGeneratorAPI` 接口与 CLI 交互（询问、日志）——generator 不感知底层使用什么提示库，CLI 负责实现。
...
export default async function generate(
  ...
  // 1. 将插件名写入 .bundlekitrc.ts plugins[]
  ...
  if (installRequest) {
    ...
    //    独立发布的插件用具体版本 "^x.y.z"
    api.addDependency("@bundlekit/request", "workspace:^", false);
    api.log("@bundlekit/request 已加入安装队列");
  }
}
```

**`IGeneratorAPI` 接口（来自 `@bundlekit/shared-utils`）：**
...
| 层 | 负责 |
|---|---|
| `bundlekit-cli/AddCommand` | Enquirer 实例化，实现 `IGeneratorAPI`，调用 `generate(context, api)` |
| `@bundlekit/shared-utils` | 定义 `IGeneratorAPI` 接口契约 |
