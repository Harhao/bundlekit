...
bundlekit-service serve --bundler vite --https
```
...
# 非交互模式
bc create my-app -t react-ts -b vite
bc create my-app -t vue3-ts -b vite
...
| `DEVKIT_NO_PROMPT` | `1` | 跳过所有 generator 交互提示（CI 自动注入，或在 `bc create` 非 TTY 路径下自动生效） |
| `DEVKIT_SKIP_INSTALL` | `1` | 跳过依赖安装步骤（生成文件后直接退出，适合调试模板） |
| `DEVKIT_PM` | `pnpm\|yarn\|npm` | 强制使用指定包管理器（等价 `--pm`，优先级低于 CLI 参数） |
| `DEVKIT_DEP_MODE` | `npm` | 强制依赖模式：`npm` 写 `^cliVersion` |
| `DEVKIT_MONOREPO_ROOT` | 路径 | 手动指定 monorepo 根目录（覆盖自动检测） |
| `DEVKIT_AUTO_INSTALL` | `1` | 非 TTY / CI 环境自动安装缺失 bundler（等价 `bc add` 后自动 install） |
...
> **TTY 检测**：generator 内的 `api.prompt()` 只在真实 TTY 下展示，当检测到 `!process.stdout.isTTY`、`DEVKIT_NO_PROMPT=1` 或 `CI=true|1` 时自动跳过（使用默认值 `false`）。`bc create` 命令在 ink 渲染路径下会自动注入 `DEVKIT_NO_PROMPT=1`，确保创建流程不会因 generator prompt 阻塞。`bc add` 命令保留交互（面向已有项目主动追加场景）。
...
interface IGeneratorAPI {
  ...
  addDependency(pkg: string, options?: { dev?: boolean }): Promise<void>;
}
```
...
```bash
bc add mock       # 追加 mock 构建插件
bc add vue        # 追加 vue 构建插件
bc add request    # 追加 @bundlekit/request 运行时库
```
...
```bash
DEVKIT_DEP_MODE=npm bc create my-app -t react-ts
```
...
如果要分享给同事，让他们也 clone monorepo 后跑 `bc create`，或等 `@bundlekit/*` 发到 npm 后用全局 cli 创建。
...
### Q: `bc create` 时 generator 追问是否安装额外包的提示没有出现

这是正常行为。`bc create`（ink 渲染路径）在调用框架 generator 前会自动注入 `DEVKIT_NO_PROMPT=1`，跳过所有交互式提问，使用默认值（不安装）。如果你想手动触发 generator 提问，切换到 `bc add` 命令：

```bash
# bc add 保留交互
bc add react    # 会弹出"是否安装 @bundlekit/request？"
```
