 ...
 *
 * 仅当用户在真 TTY 下手动调 `bc add svelte` 时才会弹 prompt。
 */
...
export default async function generate(context: string, api: IGeneratorAPI): Promise<void> {
    addPluginToConfig(context, "@bundlekit/plugin-svelte");

    if (shouldSkipPrompt()) {
        // 默认值：installRequest = false（保守不加依赖）
        // 用户后续可手动 `bc add request` 安装
        return;
    }

    // 真 TTY 路径：加视觉分隔，避免被 spinner 残留遮蔽
    ...
    if (installRequest) {
        // 用 workspace:^ 由 cli 的 normalizeDeps 兜底转 link / ^cliVersion
