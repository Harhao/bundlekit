...
/**
 * 在 .bundlekitrc.ts / .bundlekitrc.js 的 plugins[] 数组中追加一个插件名。
 *
 * 处理三种情况：
...
export function addPluginToConfig(context: string, pluginName: string): void {
    const tsPath = path.resolve(context, ".bundlekitrc.ts");
    const jsPath = path.resolve(context, ".bundlekitrc.js");
    ...
    if (!configPath) {
        throw new Error(`未找到 .bundlekitrc.ts 或 .bundlekitrc.js，请在 ${context} 下创建配置文件`);
    }
